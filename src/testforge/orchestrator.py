"""
Orchestrator — the main thing that calls each agent in order

THE FLOW (one cycle)
    1. Author writes a test for the target code
    2. Executor runs the test on the original target
    3. Adversary generates mutations
    4. Executor runs the test against each mutation. Mutations the test FAILS on are deemed "killed"
        and the rest are deemed "survived".
    5. If any mutations have survived AND we still have budget:
        Judge critiques the test, suggests improvements, loop to 1 Otherwise, return the result.
...
def run_cycle(
    ...
    *,
    ...
    while True:
        ...
        # generate mutations in target code
        mutations = generate_mutations(target_code)
        mutations_killed = 0
        surviving_mutations = []
        # run tests generated by author on mutated target code
        for m in mutations:
        ...
        # break out of loop if supervisor says so
        stop = supervisor.should_stop()
        if not surviving_mutations or stop:
            stopped_reason = stop
            break

        # call judge to provide feedback on the test case
